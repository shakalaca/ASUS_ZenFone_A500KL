...
//ASUS_BSP breeze, show memory info+++
void asus_show_migration_types(unsigned char type)
{
...
/*void asus_show_swap_cache_info(void)
{
	save_log("%lu pages in swap cache\n", total_swapcache_pages);
	save_log("Swap cache stats: add %lu, delete %lu, find %lu/%lu\n",
		swap_cache_info.add_total, swap_cache_info.del_total,
		swap_cache_info.find_success, swap_cache_info.find_total);
	save_log("Free swap  = %ldkB\n",
		get_nr_swap_pages() << (PAGE_SHIFT - 10));
	save_log("Total swap = %lukB\n", total_swap_pages << (PAGE_SHIFT - 10));
}
*/
...
void asus_show_free_areas(unsigned int filter)
{
	int cpu;
	struct zone *zone;
	...
	for_each_populated_zone(zone) {
		...
		for_each_online_cpu(cpu) {
			...
			save_log("CPU %4d: hi:%5d, btch:%4d usd:%4d\n",
			       cpu, pageset->pcp.high,
			       pageset->pcp.batch, pageset->pcp.count);
		}
	}

	save_log("active_anon:%lu inactive_anon:%lu isolated_anon:%lu\n"
	...
	for_each_populated_zone(zone) {
		...
		save_log("lowmem_reserve[]:");
		for (i = 0; i < MAX_NR_ZONES; i++)
			save_log(" %lu", zone->lowmem_reserve[i]);
		save_log("\n");
	}
	...
	for_each_populated_zone(zone) {
		...
		for (order = 0; order < MAX_ORDER; order++) {
			save_log("%lu*%lukB ", nr[order], K(1UL) << order);
			if (nr[order])
				asus_show_migration_types(types[order]);
		}
		save_log("= %lukB\n", K(total));
	}

	save_log("%ld total pagecache pages\n", global_page_state(NR_FILE_PAGES));

	//show_swap_cache_info();
}

void asus_save_tasks_mem_info(void)
{
	struct task_struct *p;
	struct task_struct *task;
	
	save_log("[ pid ]   uid  tgid total_vm      rss cpu oom_adj oom_score_adj name\n");
	for_each_process(p) {
		...
		save_log("[%5d] %5d %5d %8lu %8lu %3u     %3d         %5d %s\n",
...
void asus_show_mem()
{
	...
	save_log("Mem-info:\n");
	asus_show_free_areas(SHOW_MEM_FILTER_NODES);
	...
	save_log("%d pages of RAM\n", total);
	save_log("%d free pages\n", free);
	save_log("%d reserved pages\n", reserved);
	save_log("%d slab pages\n", slab);
	save_log("%d pages shared\n", shared);
	save_log("%d pages swap cached\n", cached);
	asus_save_tasks_mem_info();
}
EXPORT_SYMBOL(asus_show_mem);
//ASUS_BSP breeze ---
...
void save_last_shutdown_log(char* filename)
{
    ...
    if(!IS_ERR((const void *)file_handle))
    {
        sys_write(file_handle, (unsigned char*)last_shutdown_log, strlen(last_shutdown_log));
        sys_close(file_handle);
    }
    else
    {
...
static void dump_boot_reasons(void)
{
	...
	pmic_boot_reasons = (unsigned char *)(smem_get_entry(SMEM_POWER_ON_STATUS_INFO, &smem_size));
