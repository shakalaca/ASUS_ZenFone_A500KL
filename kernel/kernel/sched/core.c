...
do_wait_for_common(struct completion *x, long timeout, int state, int iowait)
{
	if (!x->done) {
		...
		do {
			 task_thread_info(current)->pWaitingCompletion = x;//ASUSDEBUG jeffery_hu@asus.com
		...
		task_thread_info(current)->pWaitingCompletion = &fake_completion;//ASUSDEBUG jeffery_hu@asus.com
...
static int __migrate_task(struct task_struct *p, int src_cpu, int dest_cpu)
{
	...
	double_rq_lock(rq_src, rq_dest);
       ...
       //ASUS BSP Workaround for Power On/Off stuck issue+++
       //check swapper/x task and dequeue "swapper/x" and dequeue it!
       if(0 == p->pid)
