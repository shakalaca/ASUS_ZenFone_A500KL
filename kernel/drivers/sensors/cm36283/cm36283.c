...
#define CM36283_GPIO_PROXIMITY_INT		65
...
//A500KL
static int g_max_light_level = 21;
static int g_cm36283_light_map[21] = {0,50,100,200,300,400,500,650,800,1000,1500,2000,3000,4000,5000,7000,10000,12500,15000,17500,20000};
    ...
    switch (property) 
    {
		...
		if(bFirst) {
			...
			if ( g_cm36283_light_gain_calibration >= 10000000 || openfilp == false )	{
				...
				g_cm36283_light_gain_calibration = 50858;//38000;
				g_cm36283_light_shift_calibration = -2;//40;
			}else
...
{
	int err = 0;
    printk("[cm36283][als] light_interrupt_work. \n");
...
static int cm36283_read_reg(struct i2c_client* client, u8 reg, int len, void *data)
{
	...
	do{
		mutex_lock(&g_cm36283_data_as->lock);
		err = i2c_transfer(client->adapter, msg, ARRAY_SIZE(msg));
		if(err != ARRAY_SIZE(msg))
		  {
			//printk(DBGMSK_PRX_G0"[cm36283] cm36283_read_reg retries= %d\n", i);
			msleep(50);
			  }
			i++;
...
static int cm36283_write_reg(u8 reg, int data_l, int data_h, int len)
{
	...
	do{
		...
		if(err < 0)
		  {
			//printk(DBGMSK_PRX_G0"[cm36283] cm36283_write_reg retries= %d\n", i);
			msleep(50);
			  }
			i++;
		mutex_unlock(&g_cm36283_data_as->lock);
	}while((err < 0) && (i<retries));
	//ASUS_BSP --- pansy_chen "[A500KL][Sensor][NA][Spec] add  retries of cm36283 i2c read/write "

	if(err < 0)
		printk(DBGMSK_PRX_G0"[cm36283] cm36283_write_reg err: reg=0x%x, data_l=%d, data_h=%d, err = 0x%x, retries=%d\n", reg, data_l, data_h, err, i);
...
static int cm36283_gpio_init(void)
{
	...
	return 0;
err:
	printk("[cm36283][board]cm36283_gpio_init error !!--\n");
