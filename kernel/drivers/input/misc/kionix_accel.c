...
// ASUS_BSP +++ yan_sun "[A500KL][Sensor][NA][Spec] support I2C stress test"
...
#define KXTJ2_INT	//enable interrupt mode
// ASUS_BSP --- yan_sun "[A500KL][Sensor][NA][Spec] Porting gsensor kxtj2"
...
// ASUS_BSP +++ yan_sun "[A500KL][Sensor][NA][Spec] SW reset gsensor when read data err"
#define ACCEL_GRP4_CTRL_REG2 	0x1F
// ASUS_BSP --- yan_sun "[A500KL][Sensor][NA][Spec] SW reset gsensor when read data err"
...
#define ACCEL_FLAT			3
/* I2C Retry Constants */
#define KIONIX_I2C_RETRY_COUNT		50 	/* Number of times to retry i2c */
...
extern bool I2C_BUS_SUSPENDING;

struct kionix_accel_platform_data g_accel_pdata = { 	5,
													200,
													1,
													1,
...
// ASUS_BSP +++ yan_sun "[A500KL][Sensor][NA][Spec] SW reset gsensor when read data err"
static int kionix_accel_grp4_SW_RESET(struct kionix_accel_driver *acceld)
{
	int err;
	err = i2c_smbus_write_byte_data(acceld->client,ACCEL_GRP4_CTRL_REG2, 0x80);
	// Do Software reset will return a none ACK  err message, 
	// Please don't care the err return.
	msleep(50);
	return 0;
}
// ASUS_BSP --- yan_sun "[A500KL][Sensor][NA][Spec] SW reset gsensor when read data err"
...
static int kionix_accel_grp4_power_on_init(struct kionix_accel_driver *acceld)
{
	...
	//if (acceld->accel_drdy) {
		...
		//k printk("write INT_CTRL_REG1 err=%d\n",err);
		if (err < 0)
			return err;
	...
	//if (acceld->accel_drdy) {//this line will let G-sensor be the IRQ mode at the power-up
		...
		//k printk("write CTRL_REG1 err = %d\n",err);
		if (err < 0)
			return err;
	}
	else {
...
static void kionix_accel_grp4_report_accel_data(struct kionix_accel_driver *acceld)
{
	...
	if(atomic_read(&acceld->accel_enabled) > 0) {
		if(atomic_read(&acceld->accel_enable_resume) > 0)
		{
			if(I2C_BUS_SUSPENDING){
				printk("[KIONIX] --- I2C  has been suspended， not do kionix_i2c_read !!!\n");
				err = -1;
			}else {
				loop = KIONIX_I2C_RETRY_COUNT;
				while(loop) {
					...
					if(err < 0){
	// ASUS_BSP +++ yan_sun "[A500KL][Sensor][NA][Spec] SW reset gsensor when read data err"
						printk("[KIONIX] --- gsensor ready to sw reset!!!\n");
						kionix_accel_grp4_SW_RESET(acceld);
						printk("[KIONIX] --- gsensor ready to re_power_on_init!!!\n");
						kionix_accel_grp4_power_on_init(acceld);
	// ASUS_BSP --- yan_sun "[A500KL][Sensor][NA][Spec] SW reset gsensor when read data err"
						loop--;
						mdelay(KIONIX_I2C_RETRY_TIMEOUT);
					}
	...
	if(acceld->accel_drdy == 1) {
		if(I2C_BUS_SUSPENDING){
			printk("[KIONIX] --- I2C  has been suspended， not do i2c_smbus_read_byte_data !!!\n");
			err = -1;
		}else {
			loop = KIONIX_I2C_RETRY_COUNT;
			while(loop) {
...
// ASUS_BSP +++ yan_sun "[A500KL][Sensor][NA][Spec] Porting gsensor kxtj2"
static void asus_set_direction_byHWID(struct kionix_accel_driver *acceld)
{
	acceld->accel_pdata.accel_direction = 1;
}
// ASUS_BSP --- yan_sun "[A500KL][Sensor][NA][Spec] Porting gsensor kxtj2"
...
// ASUS_BSP +++ yan_sun "[A500KL][Sensor][NA][Spec] Add power_on_init when hal enable gsensor"
	...
	if (err) {
		KMSGERR(&acceld->client->dev, "%s: kionix_accel_grp4_power_on_init returned err = %d. Abort.\n", __func__, err);
	}
// ASUS_BSP --- yan_sun "[A500KL][Sensor][NA][Spec] Add power_on_init when hal enable gsensor"
						...
						const char *buf, size_t count)
{
	...
	if(kionix_strtok(buf, count, &buf2, enable_count) < 0) {
		...
		printk("[Kionix] --- gsensor enable=%lu\n",enable);
		if(enable)
...
exit:
// ASUS_BSP +++ yan_sun "[A500KL][Sensor][NA][memleak]fix memory leak
	kfree(buf2);
// ASUS_BSP --- yan_sun "[A500KL][Sensor][NA][memleak]fix memory leak
	mutex_unlock(&input_dev->mutex);
...
int ReCali_Flag = false;	
#define SUM_cycle  100


static ssize_t kionix_accel_get_newcali(struct device *dev,
				struct device_attribute *attr, char *buf)
{				
	struct i2c_client *client = to_i2c_client(dev);
	struct kionix_accel_driver *acceld = i2c_get_clientdata(client);
...
// ASUS_BSP +++ yan_sun "[A500KL][Sensor][NA][ATD] support 3A for Camera AF"
unsigned char gsensor_data[48] = "";

static ssize_t kionix_accel_get_camera_3a(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s", gsensor_data);
}

static ssize_t kionix_accel_set_camera_3a(struct device *dev,
				struct device_attribute *attr, const char *buf, size_t count)
{
	memcpy(gsensor_data,buf,count);
	gsensor_data[count] = '\0';

	return count;
}
// ASUS_BSP --- yan_sun "[A500KL][Sensor][NA][ATD] support 3A for Camera AF"

// ASUS_BSP +++ yan_sun "[A500KL][Sensor][NA][ATD] support g-sensor status"
...
static DEVICE_ATTR(cali_m, S_IRUGO|S_IWUSR, kionix_accel_get_cali, kionix_accel_set_cali);
static DEVICE_ATTR(cali_a, S_IRUGO, kionix_accel_get_newcali,NULL);
static DEVICE_ATTR(accel_status, S_IRUGO, kionix_accel_get_status, NULL);
static DEVICE_ATTR(camera_3a, S_IRUGO|S_IWUSR, kionix_accel_get_camera_3a, kionix_accel_set_camera_3a);
...
static struct attribute *kionix_accel_attributes[] = {
	...
	&dev_attr_data.attr,
	&dev_attr_cali_m.attr,
	&dev_attr_cali_a.attr,
	&dev_attr_accel_status.attr,
	&dev_attr_camera_3a.attr,
	NULL
};
// ASUS_BSP --- yan_sun "[A500KL][Sensor][NA][ATD] support g-sensor status"
...
// ASUS_BSP +++ yan_sun "[A500KL][Sensor][NA][Spec] Porting gsensor kxtj2"
#ifdef KXTJ2_INT
static int kxtj2_gpio_init(struct i2c_client *client)
{
	int rc = -EINVAL;
	int gpio_int = 0;
	printk("[kxtj2]kxtj2_gpio_init +++\n");
	
	gpio_int = of_get_named_gpio_flags(client->dev.of_node, "kxtj2,irq-gpio",0,NULL);
	rc = gpio_request(gpio_int, "kxtj2-irq");
	if (rc) {
		pr_err("%s: unable to request gpio %d (kxtj2-irq)\n",__func__, gpio_int);
		goto err;
	}
...
static int TestKXTJ2SensorI2C (struct i2c_client *apClient)
{
	...
	struct kionix_accel_driver *acceld = i2c_get_clientdata(apClient);
...
//	i2c_log_in_test_case("%s +\n",__func__);
	...
	if(ret < 0){
		i2c_log_in_test_case("%s: disable gsensor failed\n",__func__);
		lnResult = -1;
		goto exit;
	}
//	i2c_log_in_test_case("%s -\n",__func__);
...
// ASUS_BSP --- yan_sun "[A500KL][Sensor][NA][Spec] Porting gsensor kxtj2"
	...
	printk("[kxtj2]: kionix_accel_probe +++\n");
	
	g_acc_irq_gpio = of_get_named_gpio_flags(client->dev.of_node, "kxtj2,irq-gpio",0,NULL);
...
// ASUS_BSP +++ yan_sun "[A500KL][Sensor][NA][Spec] Porting gsensor kxtj2"
#ifdef KXTJ2_INT
	err = kxtj2_gpio_init(client);
	if(err < 0) {
		printk("[kxtj2]: kxtj2_gpio_init failed\n");
	}
...
// ASUS_BSP --- yan_sun "[A500KL][Sensor][NA][Spec] Porting gsensor kxtj2"
	...
	if (!accel_pdata) {
		KMSGERR(&client->dev, "platform data is NULL. Hard code platform data.\n");
		return -EINVAL;
	}
	...
	switch (err) {
		...
		case KIONIX_ACCEL_WHO_AM_I_KXCJK_1013:
			...
			//if (true){//added by Eason
				...
				acceld->accel_drdy = 1;
				//k printk("Probe() init accel_register value\n");
			}
			else
				acceld->accel_drdy = 0;
...
// ASUS_BSP +++ yan_sun "[A500KL][Sensor][NA][Spec] Porting gsensor kxtj2"
	asus_set_direction_byHWID(acceld);
// ASUS_BSP --- yan_sun "[A500KL][Sensor][NA][Spec] Porting gsensor kxtj2"
	kionix_accel_update_direction(acceld);
	...
	if (acceld->accel_drdy) {
		err = request_irq(gpio_to_irq(g_acc_irq_gpio), kionix_accel_isr, IRQF_TRIGGER_RISING, KIONIX_ACCEL_IRQ, acceld);

		if (err) {
			//KMSGERR(&acceld->client->dev, "%s: request_irq returned err = %d, irq = %d \n", __func__, err, IRQ_EINT(14));
...
// ASUS_BSP +++ yan_sun "[A500KL][Sensor][NA][Spec] support I2C stress test"
#ifdef CONFIG_I2C_STRESS_TEST
	i2c_add_test_case(client, "AccelSensorTest",ARRAY_AND_SIZE(gGSensorTestCaseInfo));
#endif
// ASUS_BSP --- yan_sun "[A500KL][Sensor][NA][Spec] support I2C stress test"
...
static struct i2c_driver kionix_accel_driver = {
	.driver = {
		...
		.owner	= THIS_MODULE,
		.of_match_table = kxtj2_match_table,// ASUS_BSP yan_sun "[A500KL][Sensor][NA][Spec] Porting gsensor kxtj2"
	},
