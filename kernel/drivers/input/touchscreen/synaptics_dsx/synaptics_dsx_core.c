		...
		struct device_attribute *attr, char *buf)
{
	...
	int retval = 0;
	retval = synaptics_rmi4_reg_read(rmi4_data,0x0A,config_id,sizeof(config_id));
...
{
	char *virtual_keys = 	__stringify(EV_KEY) ":" __stringify(KEY_BACK) ":90:1330:137:60" "\n" \
				__stringify(EV_KEY) ":" __stringify(KEY_HOME) ":360:1330:171:60" "\n" \
				__stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":630:1330:137:60" "\n" ;
...
flash_prog_mode:
	...
	build_id = rmi4_data->firmware_id;
	synaptics_rmi4_reg_read(rmi4_data,0x0A,config_id,sizeof(config_id));
...
static void synaptics_rmi4_set_params_default(struct synaptics_rmi4_data *rmi4_data)
{
	unsigned char ii;
	struct synaptics_rmi4_f1a_handle *f1a;
	struct synaptics_rmi4_fn *fhandler;
	...
	input_set_abs_params(rmi4_data->input_dev, ABS_MT_POSITION_X, 0, 720, 0, 0);
	input_set_abs_params(rmi4_data->input_dev, ABS_MT_POSITION_Y, 0, 1280, 0, 0);

#ifdef REPORT_2D_W
	input_set_abs_params(rmi4_data->input_dev, ABS_MT_TOUCH_MAJOR, 0, 29, 0, 0);
	input_set_abs_params(rmi4_data->input_dev, ABS_MT_TOUCH_MINOR, 0, 29, 0, 0);
#endif
#ifdef TYPE_B_PROTOCOL
	input_mt_init_slots(rmi4_data->input_dev, 10);
#endif
...
//ASUS_BSP Freeman: add for creating virtual_key_maps ---
	...
	synaptics_rmi4_set_params_default(rmi4_data);
...
//	i2c_log_in_test_case("TestA500KLTouchID++\n");
	...
	lnResult = synaptics_rmi4_reg_read(rmi4_data, 0x0A, buf_i2c, 1);
...
static int synaptics_rmi4_power_on(
		struct synaptics_rmi4_data *rmi4_data,
		bool on) {
	int retval;
	...
	retval = reg_set_optimum_mode_check(rmi4_data->vdd, RMI4_ACTIVE_LOAD_UA);
	if (retval < 0) {
		dev_err(&rmi4_data->pdev->dev, "Regulator vdd set_opt failed rc=%d\n", retval);
		return retval;
	}

	retval = regulator_enable(rmi4_data->vdd);
	if (retval) {
		dev_err(&rmi4_data->pdev->dev, "Regulator vdd enable failed rc=%d\n", retval);
		goto error_reg_en_vdd;
	}

	//if (rmi4_data->board->i2c_pull_up) {
		retval = reg_set_optimum_mode_check(rmi4_data->vcc_i2c, RMI4_I2C_LOAD_UA);
		if (retval < 0) {
			dev_err(&rmi4_data->pdev->dev, "Regulator vcc_i2c set_opt failed rc=%d\n", retval);
			goto error_reg_opt_i2c;
		}

		retval = regulator_enable(rmi4_data->vcc_i2c);
		if (retval) {
			dev_err(&rmi4_data->pdev->dev, "Regulator vcc_i2c enable failed rc=%d\n", retval);
...
static int synaptics_rmi4_regulator_configure(
		struct synaptics_rmi4_data *rmi4_data, 
		bool on)
{
	int retval;
	...
	rmi4_data->vdd = regulator_get(&rmi4_data->pdev->dev, "8226_l19");
	if (IS_ERR(rmi4_data->vdd)) {
		dev_err(&rmi4_data->pdev->dev, "%s: Failed to get vdd regulator\n", __func__);
		return PTR_ERR(rmi4_data->vdd);
	}

	if (regulator_count_voltages(rmi4_data->vdd) > 0) {
		retval = regulator_set_voltage(rmi4_data->vdd, RMI4_VTG_MIN_UV, RMI4_VTG_MAX_UV);
		if (retval) {
			dev_err(&rmi4_data->pdev->dev, "regulator set_vtg failed retval =%d\n", retval);
			goto err_set_vtg_vdd;
		}
	}

	//if (rmi4_data->board->i2c_pull_up) {
		rmi4_data->vcc_i2c = regulator_get(&rmi4_data->pdev->dev, "8226_lvs1");
		if (IS_ERR(rmi4_data->vcc_i2c)) {
			dev_err(&rmi4_data->pdev->dev, "%s: Failed to get i2c regulator\n", __func__);
			retval = PTR_ERR(rmi4_data->vcc_i2c);
			goto err_get_vtg_i2c;
		}

		if (regulator_count_voltages(rmi4_data->vcc_i2c) > 0) {
			retval = regulator_set_voltage(rmi4_data->vcc_i2c, RMI4_I2C_VTG_MIN_UV, RMI4_I2C_VTG_MAX_UV);
			if (retval) {
				dev_err(&rmi4_data->pdev->dev, "reg set i2c vtg failed retval =%d\n", retval);
...
//	rmi4_data->reprobe_device = synaptics_rmi4_reprobe_device;
	...
	if (retval < 0) {
		dev_err(&pdev->dev, "Failed to configure regulators\n");
		goto err_reg_configure;
	}

	retval = synaptics_rmi4_power_on(rmi4_data, true);
	if (retval < 0) {
		dev_err(&pdev->dev, "Failed to power on\n");
...
//ASUS BSP Freeman: /sys/class/switch/touch +++++
	...
	if (retval < 0){
		dev_err(&pdev->dev,"%s:  Unable to register switch dev! \n",__func__);
	}
//ASUS BSP Freeman: /sys/class/switch/touch ------
...
fail_regulator_lpm:
	...
	//if (rmi4_data->board->i2c_pull_up)
	reg_set_optimum_mode_check(rmi4_data->vcc_i2c,RMI4_I2C_LOAD_UA);
...
//	const struct synaptics_dsx_board_data *bdata = rmi4_data->hw_if->board_data;
	...
	if (rmi4_data->stay_awake) {
		pr_info("%s, stay awake return", __func__);
		return 0;
	}

	if (rmi4_data->flash_prog_mode) {
		pr_info("%s, flash_prog_mode return", __func__);
		return 0;
	}
