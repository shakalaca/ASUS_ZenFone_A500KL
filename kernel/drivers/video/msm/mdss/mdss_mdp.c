...
static int mdss_mdp_probe(struct platform_device *pdev)
{
	...
	mutex_init(&mdata->list_lock);
	mdata->pipes_cleanup[0] = 0;
	mdata->pipes_cleanup[1] = 0;
	...
	char *prop_name, struct mdss_mdp_pipe *pipe_list, u32 npipes)
{
	...
	if (arr) {
		...
		for (i = 0, j = 0; i < len; j++) {
			...
			pr_debug("%s[%d]:  pipe:%p list:%p ctrl: reg_off: 0x%x bit_off: %d\n",
				prop_name, j, (void*)pipe, (void*)&pipe->list, pipe->clk_ctrl.reg_off,
				pipe->clk_ctrl.bit_off);
			pr_debug("%s[%d]: pipe:%p list:%p status: reg_off: 0x%x bit_off: %d\n",
				prop_name, j, (void*)pipe, (void*)&pipe->list, pipe->clk_status.reg_off,
