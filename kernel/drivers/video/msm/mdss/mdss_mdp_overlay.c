				       ...
				       struct mdss_mdp_pipe **ppipe)
{
	...
	if (req->id == MSMFB_NEW_REQUEST) {
		...
		mutex_lock(&mdata->list_lock);
		//check whether this pipe is in pipes_cleanup list
		list_for_each_entry(tmp, mdata->pipes_cleanup[0], list) {
			pr_debug("tmp0=%p\n", (void*)tmp);
			if((void*)tmp == (void*)pipe){
				pipe_in_cleanup = 1;
				break;
			}
		}
		
		list_for_each_entry(tmp, mdata->pipes_cleanup[1], list) {
			pr_debug("tmp1=%p\n", (void*)tmp);
			if((void*)tmp == (void*)pipe){
				pipe_in_cleanup = 1;
				break;
		...
		pr_debug("mfd index:%d, add used pipe:%p, list:%p, list:[%p<-->%p], pipe_in_cleanup=%d\n", 
				mfd->index, (void*)pipe, (void*)(&pipe->list), pipe->list.next, pipe->list.prev, pipe_in_cleanup);
		mutex_unlock(&mdata->list_lock);
		pipe->mixer = mixer;
		pipe->mfd = mfd;
...
static void mdss_mdp_overlay_cleanup(struct msm_fb_data_type *mfd)
{
	...
	pr_debug("mfd index:%d, destroy_pipes list:%p\n", mfd->index,  (void*)&destroy_pipes);

	mutex_lock(&mdata->list_lock);
	list_for_each_entry_safe(pipe, tmp, &mdp5_data->pipes_cleanup, list) {
		pr_debug("traverse pipes_cleanup:%p, list:[%p<-->%p]\n", 
			(void*)pipe, pipe->list.next, pipe->list.prev);
	...
	list_for_each_entry_safe(pipe, tmp, &destroy_pipes, list) {
		...
		mdss_mdp_pipe_destroy(pipe);
		//pr_debug("traverse destroy pipe:%p, ctrl:[0x%x, %d], list:[%p<-->%p]\n", 
		//	(void*)pipe, pipe->clk_ctrl.reg_off, pipe->clk_ctrl.bit_off, pipe->list.next, pipe->list.prev);
	}
	mutex_unlock(&mdata->list_lock);
}
	...
	u32 type)
{
	...
	for (i = 0; i < npipes; i++) {
		...
		if (pipe->is_handed_off) {
			...
			pr_debug("mfd index:%d, add cleanup pipe:%p, list:%p, list:[%p<-->%p]\n", 
				mfd->index, (void*)pipe, (void*)(&pipe->list), pipe->list.next, pipe->list.prev);
			mdss_mdp_mixer_pipe_unstage(pipe);
...
int mdss_mdp_overlay_init(struct msm_fb_data_type *mfd)
{
	...
	pr_debug("mfd index %d, pipes_used list: %p, pipes_cleanup list:%p\n", mfd->index, (void*)&mdp5_data->pipes_used, (void*)&mdp5_data->pipes_cleanup);
	if(mfd->index < 2)
		mdp5_data->mdata->pipes_cleanup[mfd->index] = (struct list_head *)(&mdp5_data->pipes_cleanup);
