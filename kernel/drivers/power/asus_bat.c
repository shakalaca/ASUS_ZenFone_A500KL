...
//ASUS_BSP  --- Eason_Chang "add BAT earlysuspend"
//frank_tao:  use frame buffer notifier to implement earlysuspend+++
...
#define TIME_FOR_FIRST_START_BATTERY_SERVER_IN_SECONDS (1)
...
extern void AXC_BatteryService_forceReportCapacity(void);
...
static int asus_bat_get_pad_bat_capacity_byhw(void)
{
	...
	char cap[2];
	int bat_cap=0;
	AX_MicroP_getBatterySoc(cap);
	bat_cap = cap[1];
	pr_err( "[BAT]! in %s(), read bat capacity  = %d\n", __FUNCTION__,bat_cap);
#else
	int bat_cap = -1;
#endif /* CONFIG_EEPROM_PADSTATION */
...
#ifdef CONFIG_EEPROM_PADSTATION
	if(1==AX_MicroP_IsP01Connected()&&(g_i2c_bus_suspended == 0))
	{
		if((ASUS_BAT_CHARGER_USB == asus_bat_get_pad_charger_byhw())&&(result != POWER_SUPPLY_STATUS_FULL))
			result = POWER_SUPPLY_STATUS_CHARGING;
	}
#endif
   //Eason if doesn't get correct ADC Vol&Curr at first update Cap show unknow status +++ 
...
*/	
	case P01_AC_IN:
		printk(DBGMSK_BAT_INFO "[BAT] %s() +++, P01_AC_IN \r\n", __FUNCTION__);
		//msleep(1500);
		if(delayed_work_pending(&asus_bat->bat_update_work))
		{
			cancel_delayed_work_sync(&asus_bat->bat_update_work);
		}
		queue_delayed_work(asus_bat_wq, &asus_bat->bat_update_work, HZ * 7);
		break;
	case P01_USB_IN:	
		printk(DBGMSK_BAT_INFO "[BAT] %s() +++, P01_USB_IN \r\n", __FUNCTION__);
		//msleep(1500);
		if(delayed_work_pending(&asus_bat->bat_update_work))
		{
			cancel_delayed_work_sync(&asus_bat->bat_update_work);
	...
	unsigned long len, void *data)
{
	...
	AXC_BatteryService_forceReportCapacity();
...
#endif*/
extern int get_BMS_SOC(void);
extern int RSOC;

extern int calculate_FCC_and_RM(int *data);
static ssize_t readBatteryFCC(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	int FCC_RM_data[2];
	calculate_FCC_and_RM(FCC_RM_data);
	return snprintf(page, PAGE_SIZE,"%d",FCC_RM_data[0]);
}
static ssize_t readBatteryRM(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	int FCC_RM_data[2];
	calculate_FCC_and_RM(FCC_RM_data);
	return snprintf(page, PAGE_SIZE,"%d",FCC_RM_data[1]);
}
static ssize_t readBatteryBMSSOC(char* page,char** start, off_t off, int count, int* eof, void* data)
{

	return snprintf(page, PAGE_SIZE,"%d",get_BMS_SOC());
}
static ssize_t readBatteryRSOC(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	return snprintf(page, PAGE_SIZE,"%d",RSOC);
}
#ifdef CONFIG_EEPROM_PADSTATION
static ssize_t readPadBatVoltage(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	char bat_info[6];
	...
	return snprintf(page, PAGE_SIZE,"%d",bat_info[0] + (bat_info[1] << 8));
}
static ssize_t readPadBatCurrent(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	char bat_info[6];
	int bat_current;
	int flag;
	...
	bat_current=bat_info[2] + (bat_info[3] << 8);
	flag=bat_info[3]&0x80;
	if(flag==0)
		return snprintf(page, PAGE_SIZE,"%d",bat_current);
	else
	{
		bat_current=0x10000-bat_current;
		return snprintf(page, PAGE_SIZE,"%d",bat_current*(-1));
	}
}
static ssize_t readPadBatRSOC(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	char cap[2];
	...
	return snprintf(page, PAGE_SIZE,"%d",cap[0]);
}
static ssize_t readPadBatUSOC(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	char cap[2];
	if(AX_MicroP_getBatterySoc(cap) < 0){
		cap[0] = 0xFF;
	...
	return snprintf(page, PAGE_SIZE,"%d",cap[1]);
}
static ssize_t readPadTemp(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	char bat_info[6];
	if(AX_MicroP_getBatteryInfo(bat_info) < 0){
		int i;
	...
	return snprintf(page, PAGE_SIZE,"%d",bat_info[4]);
}
static ssize_t readPadMode(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	char bat_info[6];
	if(AX_MicroP_getBatteryInfo(bat_info) < 0){
		int i;
	...
	return snprintf(page, PAGE_SIZE,"%d",bat_info[5]);
}
#endif
static void freg_create_proc(void)
{
	struct proc_dir_entry *BatteryFCC;
	struct proc_dir_entry *BatteryRM;
	struct proc_dir_entry *BatteryBMSSOC;
	struct proc_dir_entry *BatteryRSOC;
#ifdef CONFIG_EEPROM_PADSTATION	
	struct proc_dir_entry *PadBatVoltage;
	struct proc_dir_entry *PadBatCurrent;
	struct proc_dir_entry *PadBatRSOC;
	struct proc_dir_entry *PadBatUSOC;
	struct proc_dir_entry *PadTemp;
	struct proc_dir_entry *PadMode;
#endif

	BatteryFCC = create_proc_entry("FCC",0,NULL);
	if(BatteryFCC)
		BatteryFCC->read_proc = readBatteryFCC;

	BatteryRM = create_proc_entry("RM",0,NULL);
	if(BatteryRM)
		BatteryRM->read_proc = readBatteryRM;

	BatteryBMSSOC = create_proc_entry("BMSSOC",0,NULL);
	if(BatteryBMSSOC)
		BatteryBMSSOC->read_proc = readBatteryBMSSOC;

	BatteryRSOC = create_proc_entry("RSOC",0,NULL);
	if(BatteryRSOC)
		BatteryRSOC->read_proc = readBatteryRSOC;

#ifdef CONFIG_EEPROM_PADSTATION
	PadBatVoltage = create_proc_entry("Pad_Bat_Voltage",0,NULL);
	if(PadBatVoltage)
		PadBatVoltage->read_proc = readPadBatVoltage;

	PadBatCurrent = create_proc_entry("Pad_Bat_Current",0,NULL);
	if(PadBatCurrent)
		PadBatCurrent->read_proc = readPadBatCurrent;

	PadBatRSOC = create_proc_entry("Pad_Bat_RSOC",0,NULL);
	if(PadBatRSOC)
		PadBatRSOC->read_proc = readPadBatRSOC;

	PadBatUSOC = create_proc_entry("Pad_Bat_USOC",0,NULL);
	if(PadBatUSOC)
		PadBatUSOC->read_proc = readPadBatUSOC;

	PadTemp = create_proc_entry("Pad_Temp",0,NULL);
	if(PadTemp)
		PadTemp->read_proc = readPadTemp;

	PadMode = create_proc_entry("Pad_Mode",0,NULL);
	if(PadMode)
		PadMode->read_proc = readPadMode;
#endif
...
{
	remove_proc_entry("BatteryFCC",NULL);
	remove_proc_entry("BatteryRM",NULL);
	remove_proc_entry("BatteryBMSSOC",NULL);
	remove_proc_entry("BatteryRSOC",NULL);
#ifdef CONFIG_EEPROM_PADSTATION
	remove_proc_entry("PadBatVoltage",NULL);
	remove_proc_entry("PadBatCurrent",NULL);
	remove_proc_entry("PadBatRSOC",NULL);
	remove_proc_entry("PadBatUSOC",NULL);
	remove_proc_entry("PadTemp",NULL);
	remove_proc_entry("PadMode",NULL);
#endif
}
