				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_update_bits(codec, TAPAN_A_SPKR_DRV_EN, 0x80, 0x80);
		//+++ tyree_liu@asus.com for icepower
...
{
	int state;//asus_bsp tyree_liu@asus.com++
	disable_irq(gpio_to_irq(wcd9306_hs_data.button_gpio));
	hook_irq_balance += 1;
	state = !gpio_get_value(wcd9306_hs_data.jack_gpio);
    //asus_bsp +++ tyree_liu@asus.com;for reduce headset noise;
    if(state == 0)
...
static struct delayed_work g_amp_dwork;  //tyree_liu@asus.com for icepower on
static struct work_struct  g_amp_work;   //tyree_liu@asus.com for icepower off
...
static ssize_t audio_debug_proc_write(struct file *filp, const char *buff, size_t len, loff_t *off) {
    ...
    //+++ tyree_liu@asus.com add for icepower
