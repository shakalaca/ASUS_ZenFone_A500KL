...
#include <linux/kernel.h> // ASUS BSP +++ add for PCB-ID
...
static ssize_t
socinfo_show_pcb(struct sys_device *dev,
...
//+++ASUS BSP add for PCB-ID
static ssize_t
msm_get_pcb_version(struct device *dev,
			       struct device_attribute *attr,
			       char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n",
...
static struct sysdev_attribute socinfo_pcb_version_files[] = {
	_SYSDEV_ATTR(pcbid_status, 0444, socinfo_show_pcb, NULL),
};
//---ASUS BSP add for PCB-ID
...
//+++ASUS BSP add for PCB-ID
static struct device_attribute msm_soc_attr_pcb_revision =
